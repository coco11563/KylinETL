...
CREATE TABLE check_in_table(weibo_id String, content String, json_file String, user_id String, time_id String, city_id String, province_id String, country_id String) COMMENT 'CHECK IN DATA STORE' ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' STORED BY 'org.apache.hadoop.hive.hbase.HBaseStorageHandler' WITH SERDEPROPERTIES ("hbase.columns.mapping" = "weibo_id:content,weibo_id:json_file, weibo_id:user_id, weibo_id:time_id ,weibo_id:city_id, weibo_id:province_id, weibo_id:country_id");
