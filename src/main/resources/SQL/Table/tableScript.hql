...
CREATE TABLE check_in_table(weibo_id String, content String, geohash String,user_id String, time_id string, lo_id String , unix_time timestamp, pic_url string,lat double, lon double) COMMENT 'CHECK IN DATA STORE' ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' STORED BY 'org.apache.hadoop.hive.hbase.HBaseStorageHandler' WITH SERDEPROPERTIES ("hbase.columns.mapping" = "weibo_id:content,weibo_id:geohash, weibo_id:user_id, weibo_id:time_id ,weibo_id:lo_id, weibo_id:unix_time,weibo_id:pic_url, weibo_id:lat#b,weibo_id:lon#b");
